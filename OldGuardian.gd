...
onready var old_guardian = $AnimatedSprite
...
func idle():
	old_guardian.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	old_guardian.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	old_guardian.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		old_guardian.flip_h = direction < 0


func death():
	old_guardian.play("Death")
	motion.x = 0
