...
onready var death_brindger = $AnimatedSprite
...
func idle():
	death_brindger.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	death_brindger.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	death_brindger.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		death_brindger.flip_h = direction < 0


func death():
	death_brindger.play("Death")
	motion.x = 0
