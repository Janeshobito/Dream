...
onready var death_ripper = $AnimatedSprite
...
func idle():
	death_ripper.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	death_ripper.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	death_ripper.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		death_ripper.flip_h = direction < 0


func death():
	death_ripper.play("Death")
	motion.x = 0
