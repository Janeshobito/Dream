...
onready var electric_knight = $AnimatedSprite
...
func idle():
	electric_knight.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	electric_knight.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	electric_knight.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		electric_knight.flip_h = direction < 0


func death():
	electric_knight.play("Death")
	motion.x = 0
