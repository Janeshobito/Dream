...
onready var fire_knight = $AnimatedSprite
...
func idle():
	fire_knight.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	fire_knight.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	fire_knight.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		fire_knight.flip_h = direction < 0


func death():
	fire_knight.play("Death")
	motion.x = 0
