...
onready var metal_knight = $AnimatedSprite
...
func idle():
	metal_knight.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	metal_knight.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	metal_knight.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		metal_knight.flip_h = direction < 0


func death():
	metal_knight.play("Death")
	motion.x = 0
