...
onready var wind_knight = $AnimatedSprite
...
func idle():
	wind_knight.play("Idle")
	motion.x = 0

func attack():
	motion.x = 0
	wind_knight.play("Attack")

func run(delta):
	hero_direction()
	var direction = hero_position
	wind_knight.play("Run")
	motion.x = direction * speed + delta
	if hero.position.y < (position.y - 11):
		idle()
	else:
		wind_knight.flip_h = direction < 0


func death():
	wind_knight.play("Death")
	motion.x = 0
